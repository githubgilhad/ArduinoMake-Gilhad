ARDUINO_QUIET ?= 1	# do not show config
D_DATE_ ?= $(shell date "+%Y.%m.%d")
...
#    $(info TARGET[$(TARGET)] CURDIR[$(CURDIR)] strip[$(strip $(CURDIR))] subst[$(subst $(space),_,$(strip $(CURDIR)))] notdir[$(notdir $(subst $(space),_,$(strip $(CURDIR))))])
endif
...
ifeq ($(strip $(NO_CORE)),)
    ifndef MONITOR_BAUDRATE
        ifeq ($(words $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS)), 1)
            SPEED = $(shell grep -E -h 'Serial.begin *\([0-9]+\)' $(LOCAL_PDE_SRCS) $(LOCAL_INO_SRCS) | sed -e 's/[^0-9]//g'| head -n1)
...
CPPFLAGS      += -$(MCU_FLAG_NAME)=$(MCU) -DF_CPU=$(F_CPU) -DARDUINO=$(ARDUINO_VERSION) -DARDUINO_$(BOARD) $(ARDUINO_ARCH_FLAG) \
         ...
         "-DHEX_MAXIMUM_SIZE=$(HEX_MAXIMUM_SIZE)" \
        ...
        -fdata-sections "-D_DATE_=\"$(D_DATE_)\""
...
#       avr_size = $(SIZE) $(2)
       # We have a plain-old binutils version - just give it both.
       avr_size = $(SIZE) $(1) $(2)|$(ARDMK_DIR)/bin/myavrsize.py
